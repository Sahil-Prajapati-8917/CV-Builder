...
export default function BuilderPage() {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#09090b]">
      {/* ===== HEADER ===== */}
      <header className="glass-dark border-b border-white/[0.06] px-4 py-2.5 flex items-center justify-between shrink-0 z-50">
        <div className="flex items-center gap-3">
          {/* Sidebar Toggle */}
          <button onClick={() => setSidebarOpen(!sidebarOpen)}
            className="hidden lg:flex w-8 h-8 rounded-lg bg-white/[0.03] border border-white/[0.06] items-center justify-center text-zinc-500 hover:text-white hover:bg-white/5 transition-all">
            {sidebarOpen ? <PanelLeftClose className="h-4 w-4" /> : <PanelLeft className="h-4 w-4" />}
          </button>

          {/* Logo */}
          <Link href="/" className="flex items-center gap-2 group">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-lg shadow-violet-500/30 group-hover:shadow-violet-500/50 transition-shadow duration-500">
              <FileText className="h-4 w-4 text-white" />
            </div>
            <span className="font-bold text-sm text-white hidden sm:block">CVForge</span>
          </Link>

          <div className="w-px h-6 bg-zinc-800 hidden md:block" />

          {/* Template Selector */}
          <div className="hidden md:flex items-center gap-0.5 bg-white/[0.03] p-1 rounded-lg border border-white/[0.06]">
            {templates.map((t) => (
              <button key={t.id} onClick={() => updateCurrentCV({ template: t.id })}
                className={`px-2.5 py-1 text-[11px] font-medium rounded-md transition-all duration-300 ${
                  currentCV.template === t.id ? `text-white shadow-sm ${t.color}` : "text-zinc-500 hover:text-zinc-300 hover:bg-white/5"
                }`}>
                {t.name}
              </button>
            ))}
        ...
        <div className="flex items-center gap-1.5">
          <Button variant="ghost" size="sm" onClick={handleShare} disabled={isSaving}
            className="hidden sm:flex text-[11px] text-zinc-500 hover:text-white hover:bg-white/5 h-8 px-3">
            <Share2 className="h-3.5 w-3.5 mr-1" /> Share
          </Button>
          <Button variant="outline" size="sm" onClick={handleExportPDF} disabled={isExporting}
            className="text-[11px] h-8 px-3 border-zinc-800 text-zinc-400 hover:text-white hover:bg-white/5 hover:border-zinc-600">
            <Download className="h-3.5 w-3.5 mr-1" />
            {isExporting ? "..." : `PDF (${currentPaper.label})`}
          </Button>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* ===== STEP SIDEBAR ===== */}
        <aside className={`hidden lg:flex flex-col border-r border-zinc-800/50 bg-[#09090b] shrink-0 transition-all duration-300 overflow-hidden ${
          ...
          <div className="p-4 flex-1 overflow-y-auto min-w-[224px]">
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] font-semibold text-violet-400 tracking-wider uppercase">Progress</span>
                <span className="text-[10px] font-semibold text-zinc-500">{progressPercent}%</span>
              </div>
              <div className="h-1 bg-zinc-800 rounded-full overflow-hidden">
                <div className="h-full bg-gradient-to-r from-violet-600 to-purple-500 rounded-full transition-all duration-700 shadow-lg shadow-violet-600/50"
                ...
                return (
                  <button key={step.id} onClick={() => setCurrentStep(step.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all duration-300 group ${
                      isActive ? "bg-violet-500/10 border border-violet-500/20" : "hover:bg-white/[0.03] border border-transparent"
                    }`}>
                    <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 transition-all duration-300 ${
                      isActive ? "bg-violet-600 text-white shadow-lg shadow-violet-600/30"
                        ...
                        : "bg-zinc-800/80 text-zinc-600 group-hover:text-zinc-400"
                    }`}>
                      <Icon className="h-3.5 w-3.5" />
                    </div>
                    <div className="min-w-0">
                      <div className={`text-xs font-medium truncate ${isActive ? "text-white" : isDone ? "text-violet-300" : "text-zinc-500 group-hover:text-zinc-300"}`}>
                        {step.name}
                      </div>
                      <div className={`text-[10px] truncate mt-0.5 ${isActive ? "text-zinc-400" : "text-zinc-700"}`}>
          ...
          {/* Bottom Nav */}
          <div className="p-4 border-t border-zinc-800/50 space-y-2 min-w-[224px]">
            <Button variant="outline" onClick={prevStep} disabled={currentStep === 1}
              className="w-full gap-1.5 text-[11px] font-medium border-zinc-800 text-zinc-500 hover:text-white hover:bg-white/5 hover:border-zinc-600 h-9">
              <ChevronLeft className="h-3.5 w-3.5" /> Previous
            </Button>
            {currentStep < 7 ? (
              <Button onClick={handleNext}
                className="w-full gap-1.5 bg-gradient-to-r from-violet-600 to-purple-600 hover:from-violet-500 hover:to-purple-500 text-[11px] font-semibold h-9 shadow-lg shadow-violet-600/25">
        ...
        <div className={`${showMobilePreview ? "hidden lg:flex" : "flex"} flex-1 flex-col overflow-hidden lg:w-auto w-full`}>
          <div className="flex-1 overflow-y-auto">
            <div className="max-w-2xl mx-auto px-6 py-8">
              ...
              <div className="lg:hidden mb-6">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-semibold text-violet-400">Step {currentStep}</span>
                    <span className="text-zinc-700">•</span>
                    <span className="text-xs text-zinc-500">{STEPS[currentStep - 1].name}</span>
                  </div>
                  <span className="text-[10px] text-zinc-600">{progressPercent}%</span>
                </div>
                <div className="flex gap-1">
                  {STEPS.map((s) => (
                    <button key={s.id} onClick={() => setCurrentStep(s.id)}
                      className={`flex-1 h-1 rounded-full transition-all duration-500 ${
                        s.id < currentStep ? "bg-violet-500" : s.id === currentStep ? "bg-violet-600" : "bg-zinc-800"
                      }`} />
                  ))}
                </div>
              </div>
              ...
              <div className="mb-8">
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500/10 to-purple-500/10 border border-violet-500/20 flex items-center justify-center">
                    {(() => { const Icon = stepIcons[currentStep - 1]; return <Icon className="h-5 w-5 text-violet-400" />; })()}
                  </div>
                  <div>
                    <h1 className="text-lg font-bold text-white">{STEPS[currentStep - 1].name}</h1>
                    <p className="text-xs text-zinc-500">{stepDescriptions[currentStep - 1]}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Mobile Nav */}
              <div className="lg:hidden flex justify-between items-center mt-10 pt-6 border-t border-zinc-800/50">
                <Button variant="outline" onClick={prevStep} disabled={currentStep === 1}
                  className="gap-1.5 text-sm font-medium border-zinc-700 text-zinc-400 hover:text-white hover:bg-white/5">
        ...
        {/* ===== PREVIEW PANEL ===== */}
        <div className={`${showMobilePreview ? "flex" : "hidden"} lg:flex flex-col w-full lg:w-[55%] bg-zinc-950 border-l border-zinc-800/50`}>
          {/* Preview Header */}
          <div className="px-4 py-2.5 glass-dark border-b border-zinc-800/50 flex items-center justify-between shrink-0">
            <div className="flex items-center gap-2">
              <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
              <span className="text-[11px] font-semibold text-zinc-400">Live Preview</span>
            </div>
            <div className="flex items-center gap-1">
              {/* Zoom */}
              <button onClick={() => setZoom(z => Math.max(0.4, z - 0.1))}
                className="w-7 h-7 rounded-md bg-white/5 hover:bg-white/10 flex items-center justify-center text-zinc-500 hover:text-white transition-all">
              ...
              <div className="relative">
                <button onClick={() => setShowSizeMenu(!showSizeMenu)}
                  className="h-7 px-2 rounded-md bg-white/5 hover:bg-white/10 flex items-center gap-1 text-zinc-500 hover:text-white transition-all text-[10px] font-medium">
                  {currentPaper.label}
                  <ChevronRight className="h-3 w-3 rotate-90" />
                </button>
                {showSizeMenu && (
                  <div className="absolute top-full right-0 mt-1 bg-zinc-900 border border-zinc-700 rounded-lg shadow-2xl shadow-black/50 overflow-hidden z-50 min-w-[100px]">
                    {Object.entries(paperSizes).map(([key, size]) => (
                      <button key={key} onClick={() => { setPaperSize(key); setShowSizeMenu(false); }}
                        className={`w-full px-3 py-2 text-left text-[11px] hover:bg-violet-500/10 transition-all flex items-center justify-between ${
                          paperSize === key ? "text-violet-400 bg-violet-500/5" : "text-zinc-400 hover:text-white"
                        }`}>
                        <span>{size.label}</span>
                        <span className="text-[9px] text-zinc-600">{size.w}x{size.h}mm</span>
                      </button>
                    ))}
                  </div>
                )}
              ...
              <button onClick={() => setShowMobilePreview(false)}
                className="lg:hidden w-7 h-7 rounded-md bg-white/5 hover:bg-white/10 flex items-center justify-center text-zinc-500 hover:text-white transition-all ml-1">
          ...
          {/* Preview Content */}
          <div ref={previewContainerRef} className="flex-1 overflow-auto p-6 flex justify-center"
            onClick={() => setShowSizeMenu(false)}>
            <div
              style={{ transform: `scale(${zoom})`, transformOrigin: "top center" }}
              className={`shadow-2xl rounded-lg overflow-hidden bg-white shadow-black/50 ring-1 ring-white/5 transition-all duration-300 w-[595px] ${currentPaper.ratio}`}
            >
