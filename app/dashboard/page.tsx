      ...
      <header className="bg-white/80 dark:bg-[#09090b]/80 backdrop-blur-2xl border-b border-gray-200 dark:border-white/[0.06] sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <ThemeToggle />
            <Link href="/templates" className="text-sm text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white transition-colors duration-300">Templates</Link>
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
        ...
        {cvs.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {cvs.map((cv) => (
              <div key={cv.id} className="group rounded-2xl overflow-hidden border border-gray-200 dark:border-zinc-800/50 hover:border-violet-300 dark:hover:border-violet-500/30 hover:shadow-xl hover:shadow-violet-500/10 transition-all duration-500 bg-white dark:bg-zinc-900/30">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex gap-1.5">
                      <Button variant="outline" size="sm" onClick={() => handleEdit(cv.id!)}
                        className="flex-1 text-violet-600 dark:text-violet-400 border-violet-200 dark:border-violet-500/20 hover:bg-violet-50 dark:hover:bg-violet-500/10 hover:border-violet-300 dark:hover:border-violet-500/40 text-xs font-medium">
                        <Edit className="h-3 w-3 mr-1" /> Edit
                      </Button>
                      <Button variant="outline" size="sm" onClick={() => copyShareLink(cv.slug!)}
                        className="px-2 border-gray-200 dark:border-zinc-700 text-gray-500 dark:text-zinc-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-white/5 hover:border-gray-300 dark:hover:border-zinc-500" title="Copy share link">
            ...
            <button onClick={handleNew}
              className="min-h-[300px] rounded-2xl border-2 border-dashed border-gray-300 dark:border-zinc-800 hover:border-violet-400 dark:hover:border-violet-500/40 bg-gray-50/50 dark:bg-zinc-900/20 hover:bg-violet-50 dark:hover:bg-violet-500/5 flex flex-col items-center justify-center gap-4 transition-all duration-500 group">
